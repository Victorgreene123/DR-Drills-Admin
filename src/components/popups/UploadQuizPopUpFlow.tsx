...
const UploadQuizPopUpFlow: React.FC<UploadQuizPopUpFlowProps> = ({
  ...
  const [isQuizBlockOpen, setIsQuizBlockOpen] = useState(false);
  const [selectedQuizBlock, setSelectedQuizBlock] = useState("Quiz Block");
  const [quizBlockSearchTerm, setQuizBlockSearchTerm] = useState<string>("");
  ...
  const fastTracks = [
    {
      label: "Select From Lectures",
      onClick: () => setIsLecturePopupOpen(true)
    },
    {
      label: "Upload new Lecture",
      onClick : () => {
        console.log("Upload new Lecture clicked");
      }
    }
  ...
  const [selectedMode, setSelectedMode] = useState("Quiz Mode only");
  ...
  const filteredQuizBlocks = quizBlocks.filter((item) =>
    item.toLowerCase().includes(quizBlockSearchTerm.toLowerCase())
  );
...
const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#00000066] flex items-center justify-center z-[1000]">
      <div className="bg-white rounded-[16px] w-[800px] shadow-lg relative">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-1 border-b border-[#C3C6CF] rounded-t-[16px] bg-[#F8F9FF]">
        ...
        {step === "details" && selectedFile && (
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div className="relative p-2 h-[56px]">
                  <textarea
                    className="w-full absolute top-0 left-0 border-[1px] border-[#73777F] outline-none rounded h-full px-2 py-1 text-xs"
                    placeholder="Title"
                    style={{ resize: "none" }}
                  />
                  <p className="absolute bottom-0 text-[#73777F] text-[11px] right-2">2/100</p>
                </div>

                {/* Course Dropdown */}
                <div className="w-full relative border border-[#73777F] rounded-[6px] text-xs min-h-[28px]">
                  {!isDropped ? (
                ...
                <div className="w-full relative border border-[#73777F] rounded-[6px] text-xs px-2 py-1 h-[28px] flex items-center justify-between cursor-pointer min-h-[28px]">
                  <p>Source</p>
                  <FaChevronDown className="text-[#73777F] text-xs" />
                </div>

                {/* Tags Dropdown (static) */}
                <div className="w-full relative border border-[#73777F] rounded-[6px] text-xs px-2 py-1 flex items-center justify-between cursor-pointer min-h-[28px]">
                  <p>Tags</p>
                  <FaChevronDown className="text-[#73777F] text-xs" />
                </div>
              ...
              <div className="space-y-2">
                <div className="border border-[#C3C6CF] rounded p-2">
                  <img
                    src="https://via.placeholder.com/250x150?text=CSV+Preview"
                    alt="Uploaded file"
                    className="rounded"
                  />
          ...
          <div className="w-1/2 space-y-1">
              <label className="block text-xs font-medium">Mode</label>
            ...
            {/* Quiz Block Dropdown */}
            <div className="w-1/2 space-y-1">
              <label className="block text-xs font-[500] text-[#1A1C1E] font-medium">Quiz Block</label>
              ...
              <div className="w-full relative border border-[#73777F] rounded-[6px] text-xs min-h-[28px]">
                {!isQuizBlockOpen ? (
                  <div className="px-2 py-1 flex items-center justify-between w-full cursor-pointer min-h-[28px]" onClick={() => setIsQuizBlockOpen(true)}>
                    <p>{selectedQuizBlock}</p>
                    <FaChevronRight className="text-[#73777F] text-xs" />
                  </div>
                ) : (
                  <div className="w-full h-[28px] relative">
                    ...
                    <input
                      type="text"
                      className="absolute top-0 h-full w-full outline-none border-none left-0 pl-7 text-xs"
                      placeholder="Search quiz block"
                      autoFocus
                      onChange={e => setQuizBlockSearchTerm(e.target.value)}
                      value={quizBlockSearchTerm}
                    />
                    <button onClick={() => setIsQuizBlockOpen(false)}>
                      <FaChevronUp className="text-[#73777F] text-xs absolute top-2 right-2" />
                    </button>
                  </div>
                )}
                {isQuizBlockOpen && filteredQuizBlocks.length !== 0 && (
                  <div className="absolute top-8 left-0 bg-white border border-[#C3C6CF] rounded shadow-lg w-full z-10">
                    <ul className="p-1 max-h-32 overflow-y-auto">
                      {filteredQuizBlocks.map((item) => (
                        <li
                          className="py-1 px-2 hover:bg-[#F2F3FA] cursor-pointer text-xs"
                          key={item}
                          onClick={() => {
                            setSelectedQuizBlock(item);
                            setIsQuizBlockOpen(false);
                            setQuizBlockSearchTerm("");
                          }}
                        >
                          {item}
                        </li>
            ...
            {/* Fast Track Dropdown */}
            <div className="w-1/2 space-y-1">
              <label className="block text-xs font-medium">Fast track</label>
              <p className="text-black font-[400] text-[10px]">Use this quiz as a post-lecture quiz in a lecture</p>
              
              <div className="w-full relative border border-[#73777F] rounded-[6px] text-xs min-h-[28px]">
                 
                  <div className="px-2 py-1 flex items-center justify-between w-full cursor-pointer min-h-[28px]" onClick={() => setIsFastTrackOpen(true)}>
                    <p>{selectedFastTrack}</p>
                    <FaChevronRight className="text-[#73777F] text-xs" />
                  </div>
                    ...
                    <ul className="p-1 max-h-32 overflow-y-auto">
                      {fastTracks.map((item) => (
                        <li
                          ...
                          key={item.label}
                          onClick={() => {
                            setSelectedFastTrack(item.label);
                            setFastTrackSearchTerm("");
            ...
            <div className="w-1/2 space-y-1">
              <label className="block text-xs font-medium">User Restriction</label>
      ...
      {isLecturePopupOpen && (
        <SelectLecturePopup
          ...
          onSelectLecture={(lecture: string , lectureid : any) => {
            ...
            setSelectedFastTrack(lecture);
            setIsLecturePopupOpen(false);
